...
# Codex改动：支持 -J JSON 输出，保持批量检测脚本调用方式兼容。
if [[ " $* " == *" -J "* ]]; then
    ...
    UNLOCK_JSON_MODE=1 bash "$0" "${json_args[@]}" | "$script_dir/parse_unlock_result.py"
    exit ${PIPESTATUS[0]}
fi
...
validate_proxy() {
    ...
    # Codex改动：批量检测使用 socks5h://，这里同步放行 socks5h。
    local tmpresult=$(echo "$1" | grep -P '^(socks|socks4|socks5|socks5h|http)://([^:]+:[^@]+@)?(([0-9]{1,3}\.){3}[0-9]{1,3}|(\[[0-9a-fA-F:]+\]|([0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}|((([0-9a-fA-F]{1,4}:){1,6})|::(([0-9a-fA-F]{1,4}:){1,6}))([0-9a-fA-F]{1,4}))):(0|[1-9][0-9]{0,4})$')
    if [ -z "$tmpresult" ]; then
...
process() {
    ...
    if [ -n "$proxy" ]; then
        ...
        if validate_proxy "$proxy"; then
            USE_PROXY="-x $proxy"
            # Codex改动：批量代理检测只走代理出口 IPv4，避免 SOCKS 代理下重复触发 IPv6 分支。
            NETWORK_TYPE=4
        fi
    fi

    if [ -z "$netType" ] && [ -z "$NETWORK_TYPE" ]; then
        NETWORK_TYPE=''
    fi
...
function WebTest_OpenAI() {
    ...
    # Codex改动：ChatGPT 区域查询必须走代理，避免显示本机出口国家码。
    local countryCode=$(curl ${CURL_DEFAULT_OPTS} -s 'https://chat.openai.com/cdn-cgi/trace' --user-agent "${UA_BROWSER}" | grep '^loc=' | awk -F= '{print $2}')
    if [ -n "$countryCode" ]; then
        local regionSuffix=" (Region: ${countryCode})"
    else
        local regionSuffix=""
    fi
    if [ -z "$result2" ] && [ -z "$result1" ]; then
        echo -n -e "\r ChatGPT:\t\t\t\t${Font_Green}Yes${regionSuffix}${Font_Suffix}\n"
        return
    fi
    ...
    if [ -z "$result1" ] && [ -n "$result2" ]; then
        echo -n -e "\r ChatGPT:\t\t\t\t${Font_Yellow}No (Only Available with Web Browser${regionSuffix})${Font_Suffix}\n"
        return
    fi
    if [ -n "$result1" ] && [ -z "$result2" ]; then
        echo -n -e "\r ChatGPT:\t\t\t\t${Font_Yellow}No (Only Available with Mobile APP${regionSuffix})${Font_Suffix}\n"
        return
    fi
...
function Global_UnlockTest() {
    ...
    wait
    local array=("Bing Region:" "Apple Region:" "ChatGPT:" "Google Gemini:" "Claude:" "Google Play Store:" "Steam Currency:")
    echo_result ${result} ${array}
    echo "======================================="
}
...
function inputOptions() {
    ...
    while :; do
        ...
        # Codex改动：精简版不再开放区域菜单，统一只运行面板需要的检测集合。
        REGION_ID=0
        break
    done
}
...
function runScript() {
    ...
    # Codex改动：精简版固定只运行面板展示需要的检测集合，避免菜单参数触发其它区域/广告/体育检测。
    checkIPConn 4
